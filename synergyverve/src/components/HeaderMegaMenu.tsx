  ...
  return (
    <div className="relative">
      ...
      {megaMenuOpen && (
        <div 
          ref={megaMenuRef}
          className="fixed top-24 left-0 w-full bg-white/95 dark:bg-text/95 backdrop-blur-md shadow-lg overflow-hidden z-50 max-h-[80vh] overflow-y-auto"
        >
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 px-6 pb-6">
              {services.map((service) => (
                <div key={service.category} className="mb-4">
                  <p className="font-bold text-lg mb-2">{service.category}</p>
                  <div className="space-y-1">
                    {service.subcategories.map((sub) => (
                      <div key={sub.name}>
                        <a 
                          href={sub.link} 
                          className="block w-full py-1 px-2 rounded-md hover:bg-gray-100/20 no-underline text-current text-sm"
                        >
                          {sub.name}
                        </a>
                      </div>
            ...
            <div className="mt-4 p-4 bg-gray-100/10 border-t border-gray-200/20">
              <div className="flex justify-between items-center max-w-7xl mx-auto">
                <p className="font-medium text-sm">Need a custom solution?</p>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-white dark:bg-text z-50 lg:hidden pt-24 overflow-y-auto">
          <div className="px-4 py-2">
            <a href="#" className="block px-4 py-3 text-xl font-bold hover:bg-gray-100/10 dark:hover:bg-gray-800/50 rounded-lg">
              HOME
            </a>
            <a href="#" className="block px-4 py-3 text-xl font-bold hover:bg-gray-100/10 dark:hover:bg-gray-800/50 rounded-lg">
              ABOUT
